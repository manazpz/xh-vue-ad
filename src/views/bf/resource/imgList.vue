    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="70px"
        ...
        <el-form-item label-width="110px" label="类型" class="postInfo-container-item">
          <el-select v-model="temp.type" style="width: 290px" >
            <el-option v-for="item in typeOptions" :key="item.keyWord" :label="item.name" :value="item.keyWord" >
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label-width="110px" label="链接模式" class="postInfo-container-item">
          <el-select v-model="temp.bannerType" style="width: 290px" @change="onSelectedDrug($event)">
            <el-option v-for="item in bannerTypeOptions" :key="item.keyWord" :label="item.name" :value="item.keyWord" >
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getBannerType() {
        getConfig({ 'typef': 'BANNERTYPE' }).then(response => {
          this.bannerTypeOptions = response.data.items
        })
      },
